package main.java;

import javafx.util.Pair;

import java.util.Scanner;

public  class InputHelper {
    final static String inputPath = "C:\\Users\\user\\Desktop\\Учеба\\ВычМат\\ЛР6\\Lab6\\resources\\input.txt";
    final static String outputPath = "C:\\Users\\user\\Desktop\\Учеба\\ВычМат\\ЛР6\\Lab6\\resources\\output.txt";

    public static int binChoose(Scanner scanner, String question, String var1, String var2) {
        while (true) {
            int ans;
            System.out.println(question);
            System.out.println(var1 + " - нажмите 1");
            System.out.println(var2 + " - нажмите 2");
            try {
                ans = Integer.parseInt(scanner.next());
                if (ans != 1 && ans != 2) {
                    throw new IllegalArgumentException();
                }
            } catch (Exception e) {
                System.out.println("Непредвиденный ввод");
                continue;
            }
            return ans;
        }
    }

    public static class Interval{
        private double a, b;

        public Interval(Scanner scanner, String comment){
            while (true) {
                System.out.println(comment);
                try {
                    a = Double.parseDouble(scanner.next());
                    b = Double.parseDouble(scanner.next());
                    if (a > b) {
                        double temp = a;
                        a = b;
                        b = temp;
                    }
                } catch (Exception e) {
                    System.out.println("Непредвиденный ввод");
                    continue;
                }
                break;
            }
        }

        public double getA() {
            return a;
        }

        public double getB() {
            return b;
        }
    }

    public static double setDouble(Scanner scanner, String comment){
        double x;
        while(true){
            System.out.println(comment);
            try {
                x = Double.parseDouble(scanner.next());
            }
            catch (Exception e){
                System.out.println("Непредвиденный ввод");
                continue;
            }
            return x;
        }
    }


    public static int setInt(Scanner scanner, String comment){
        int x;
        while(true){
            System.out.println(comment);
            try {
                x = Integer.parseInt(scanner.next());
            }
            catch (Exception e){
                System.out.println("Непредвиденный ввод");
                continue;
            }
            return x;
        }
    }


    @Deprecated
    public static Pair<Double, Double> setInterval(Scanner scanner, String comment) {
        double a, b;
        while (true) {
            System.out.println(comment);
            try {
                a = Double.parseDouble(scanner.next());
                b = Double.parseDouble(scanner.next());
                if (a > b) {
                    double temp = a;
                    a = b;
                    b = temp;
                }
            } catch (Exception e) {
                System.out.println("Непредвиденный ввод");
                continue;
            }
            return new Pair<>(a, b);
        }
    }

    public static Pair<Double, Double> setXY(Scanner scanner) {
        double a, b;
        while (true) {
            try {
                a = Double.parseDouble(scanner.next());
                b = Double.parseDouble(scanner.next());
            } catch (Exception e) {
                System.out.println("Непредвиденный ввод");
                continue;
            }
            return new Pair<Double, Double> (a, b);
        }
    }

}
